...
static const NSUInteger kGridLines = 5;
...
//@property (nonatomic, strong) ZGDrawView *drawView;
@end
...
- (instancetype)initWithFrame:(CGRect)frame
{
        if (self = [super initWithFrame:frame]) {
                ...
                _upperLeft.center = CGPointMake(kCropViewCornerLength / kCropViewWidch, kCropViewCornerLength / kCropViewWidch);
                ...
                _upperRight.center = CGPointMake(self.frame.size.width - kCropViewCornerLength / kCropViewWidch, kCropViewCornerLength / kCropViewWidch);
                ...
                _lowerRight.center = CGPointMake(self.frame.size.width - kCropViewCornerLength / kCropViewWidch, self.frame.size.height - kCropViewCornerLength / kCropViewWidch);
                ...
                _lowerLeft.center = CGPointMake(kCropViewCornerLength / kCropViewWidch, self.frame.size.height - kCropViewCornerLength / kCropViewWidch);
...
#pragma mark - SNPEScreenshotView
@interface SNPEScreenshotView () <UIScrollViewDelegate, CropViewDelegate>
...
@implementation SNPEScreenshotView
...
#endif
                _photoContentView = [[PhotoContentView alloc] initWithFrame:self.bounds Image:image];
...
-(void)cancelBtnAction{
        [self.delegate buttonIsclick:self];
}
//保存截图
-(void)doneAction{
        [self.delegate buttonIsDone:self];
}
- (instancetype)initWithFrame:(CGRect)frame image:(UIImage *)image
{
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
        ...
        }else if(CGRectContainsPoint(self.doneBtn.frame, point)){
                return self.doneBtn;
