...
- (instancetype)initWithFrame:(CGRect)frame image:(UIImage *)image
{
        ...
        if (self) {
                self.image = image;
                self.backgroundColor = COLOR_FOR_PHOTO_EDIT_BACKGROUND;
...
- (void) initCropView{
        ...
        self.leftView.backgroundColor = COLOR_FOR_PHOTO_EDIT_BACKGROUND;
        self.leftView.userInteractionEnabled = NO;
        [self addSubview:self.leftView];
        self.rightView = [[UIView alloc] init];
        self.rightView.backgroundColor = COLOR_FOR_PHOTO_EDIT_BACKGROUND;
                ...
                if (self.cropDelegate && [self.cropDelegate conformsToProtocol:@protocol(ZGPhotoEditCropViewDelegate)]) {
                        [self.cropDelegate photoEditCropViewDidFinish:self
                                                           centerPoint:CGPointMake(
                                                                                   self.cropView.frame.origin.x + self.cropView.frame.size.width / 2 - self.scrollView.contentOffset.x * 1.1 / self.scrollView.zoomScale,
                                                                                   self.cropView.frame.origin.y + self.cropView.frame.size.height / 2 - self.scrollView.contentOffset.y * 1.1 / self.scrollView.zoomScale)
...
-(void)layoutSubviews{
        ...
        self.cancelButton.frame = CGRectMake(0, self.frame.size.height - HEIGHT_PHOTO_EIDT_BAR_TOOL_X, HEIGHT_PHOTO_EIDT_BAR_TOOL, HEIGHT_PHOTO_EIDT_BAR_TOOL);
        self.resetButton.frame = CGRectMake(self.frame.size.width / 2 - HEIGHT_PHOTO_EIDT_BAR_TOOL * 0.5,  self.frame.size.height - HEIGHT_PHOTO_EIDT_BAR_TOOL_X, HEIGHT_PHOTO_EIDT_BAR_TOOL, HEIGHT_PHOTO_EIDT_BAR_TOOL);
        
        self.finishButton.frame = CGRectMake(self.frame.size.width - HEIGHT_PHOTO_EIDT_BAR_TOOL,  self.frame.size.height - HEIGHT_PHOTO_EIDT_BAR_TOOL_X, HEIGHT_PHOTO_EIDT_BAR_TOOL, HEIGHT_PHOTO_EIDT_BAR_TOOL);
}
